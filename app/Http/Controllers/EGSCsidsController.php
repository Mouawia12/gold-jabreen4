...
class EGSCsidsController extends Controller
{
    private $egs_info;
    private $api;
    ...
    { 
        ...
        $this->api = new ApiCsidsController();
    }
	...
	public function productionCsids(string $compliance_request_id, string $certificate, string $secret): string
    {
        if (!$certificate || !$secret)
            throw new Exception('EGS is missing a certificate/private key/api secret to check the invoice compliance.');

        list($issueCertificate, $productionCsids) = $this->api->compliance($certificate, $secret);
        $issued_data = $productionCsids($compliance_request_id);
