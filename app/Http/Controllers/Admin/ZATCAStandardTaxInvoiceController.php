...
class ZATCAStandardTaxInvoiceController extends Controller
{
    ...
    {
        $this->templates = new XmlStandardTemplateController();
    }

    public function simplifiedTaxInvoice(array $invoice, array $egs_unit)
    {
        $populated_template = $this->templates->simplified_tax_invoice_template($this->ZATCAInvoiceTypes[$egs_unit['cancelation']['cancelation_type']]);
        ...
        $populated_template = str_replace('SET_PAYMENT_METHODS', $this->ZATCAPaymentMethods[$egs_unit['payment_methods']['methods1']], trim($populated_template));
        ...
        if (isset($egs_unit['cancelation']['canceled_invoice_number']) && $egs_unit['cancelation']['canceled_invoice_number']) {
            $populated_template = str_replace('SET_BILLING_REFERENCE', $this->defaultBillingReference($egs_unit['cancelation']['canceled_invoice_number'],$egs_unit['cancelation']['canceled_invoice_date']), $populated_template);
            $populated_template = str_replace('SET_INSTRUCTION_NOTE',$this->templates->invoice_billing_reference_Instruction_Note_template(), $populated_template);
        } else {
            $populated_template = str_replace('SET_INSTRUCTION_NOTE','', $populated_template);
        ...
        $populated_template = str_replace('SET_CUSTOMER_STREET', $invoice['customer_street'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_BUILDING', $invoice['customer_building'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_CITY_SUB', $invoice['customer_citySub'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_CITY', $invoice['customer_city'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_POSTAL', $invoice['customer_postal'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_VAT', $invoice['customer_vat'], $populated_template);
        $populated_template = str_replace('SET_CUSTOMER_NAME', $invoice['customer_name'], $populated_template);
    ...
    private function defaultBillingReference(string $invoice_number, string $invoice_date): string
    { 
        $populated_template = $this->templates->invoice_billing_reference_template();
        $populated_template = str_replace('SET_INVOICE_DATE', $invoice_date, $populated_template);
        $populated_template = str_replace('SET_INVOICE_NUMBER', $invoice_number, $populated_template);
    ...
    { 
        $populated_template = $this->templates->ubl_signature_signed_properties_for_signing_template();
    ...
    { 
        $populated_template = $this->templates->ubl_signature_signed_properties_template();
    ...
    public function defaultUBLExtensions(string $invoice_hash, string $signed_properties_hash, string $digital_signature, string $cleanUpCertificateString, string $ubl_signature_signed_properties_xml_string): string
    {
        ...
        $populated_template = $this->templates->ubl_signature();
...
//        }
         ...
         * set invoice lines
         */ 
        $tax_total_template = $this->templates->tax_total_template();
        ...
        foreach ($item_lines[0]['cac:TaxSubtotal'] as $line) {
            ...
            if($line['cbc:TaxAmount']['#text'] > 0){ 
                $l = str_replace('SET_TAX_EXEMPTION_REASON_CODE','', $l);
            }else{
                $l = str_replace('SET_TAX_EXEMPTION_REASON_CODE',$this->templates->tax_exemption_reason(), $l);
            }
         ...
         */ 
        $legal_monetary_total_template = $this->templates->legal_monetary_total_template();
         ...
         * set invoice lines
         */ 
        $invoice_line_template = $this->templates->invoice_line_template();
    ...
    private function constructLineItemTotals($line_item): array
    {
        ...
        //update 21-05-2024 
        $line_item_subtotal = number_format($line_item_subtotal, 2, '.', '');
