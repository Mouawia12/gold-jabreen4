...
class SystemController extends Controller
{
    ...
    public function  PurchaseCollectibleAccounting($id)
    {
        ...
        if ($bill) {
            ...
            $headerData = [
                ...
                'basedon_id' => $id,
                'baseon_text' => 'شراء مقتنيات ثمينة',
                'total_credit' => 0,
                'total_debit' => 0,
                'notes' => ''
            ]; 
    ...
    public function EnterWorkAccounting($id)
    {
        ...
        if ($bill) {
           ...
           if($bill -> bill_type == 0){   
               $gold_stock_credit_account = $settings->stock_old_account;
           }elseif($bill -> bill_type == 2){   
               $gold_stock_credit_account = $settings->stock_pure_account;
           }else{ 
               $gold_stock_credit_account = 0;
           }
            ...
            ////log_message('error','F6 :'.$id);
            if ($bill->net_money > 0 and $bill->total_money > 0) {
                $detailsData[] = [
                    'account_id' => $customerAccount,
                    //'account_id' => $settings->supplier_default_account,
                    'debit' => 0,
                    'credit' => $bill->net_money - $bill->made_total,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => ''
                ];
            } 
            ...
            if ($bill->total_money > 0) {
                $detailsData[] = [
                    'account_id' => $settings->purchase_account,
                    //'account_id' => $settings->purchase_Jewelry_account,
                    'debit' => $bill->total_money,
                    'credit' => 0, 
                    'ledger_id' => 0,
                    'notes' => ''
            ...
            //اجور التصنيع للذهب المشغول
            if ($bill->made_total > 0) {
                //مصروف اجور التصنيع
                $detailsData[] = [
                    'account_id' => $settings->made_account,
                    'debit' => $bill->made_total,
                    'credit' => 0,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => ''
                ];
                ...
                //المورد المذكور للفاتورة
                $detailsData[] = [
                    'account_id' => $customerAccount,
                    'debit' => 0,
                    'credit' => $bill->made_total + $tax,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => ''
                ];
            } 

            if( $gold_stock_credit_account > 0){

                $detailsData[] = [
                    'account_id' => $settings->stock_Jewelry_account,
                ...
                $detailsData[] = [
                    'account_id' => $gold_stock_credit_account,
                    'debit' => 0,
    ...
    public function ExitWorkTaxAccounting($id){

        $bill = ExitWorkTax::find($id);
        $details = ExitWorkTaxDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        ...
        if($bill){
            ...
            $detailsData[] = [
                'account_id' =>  $bill->type == 0 ? $settings->cost_account : $settings->stock_under_account, 
                'debit' => $bill->total21_gold,
                'credit' => 0,
                'ledger_id' => 0,
                'notes' => 'جرام ذهب عيار 21'
    ...
    public function SaleCollectibleAccounting($id){
        $bill = SaleCollectible::find($id);
        $details = SaleCollectibleDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
            $total_tax = + ($detail -> weight * $detail -> gram_tax );
        }
    ...
    public function ReturnSaleCollectibleAccounting($id){ 
        $bill = SaleCollectible::find($id);
        $details = SaleCollectibleDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
        ...
        if($bill){
            ...
            $headerData = [
                ...
                'basedon_no' => $bill->bill_number,
                'basedon_id' => $id,
                'baseon_text' => 'مرتجع بيع مقتنيات ثمينة',
                'total_credit' => 0,
                'total_debit' => 0,
                'notes' => ''
            ];
    ...
    public function ReturnExitWorkTaxAccounting($id){
        $bill = ExitWorkTax::find($id);
        $details = ExitWorkTaxDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
    ...
    public function ReturnExitOldAccounting($id){
        ...
        if($bill){
            ...
            if($bill->bill_type == 0){
                $isType = 'مرتجع بيع ذهب كسر ';
                $gold_stock_account = $settings->stock_old_account;
            }else{
                $isType = 'مرتجع بيع ذهب صافي ';
                $gold_stock_account = $settings->stock_pure_account;
            }
    ...
    public function ReturnExitOldTaxAccounting($id){
        $bill = ExitOldTax::find($id);
        $details = ExitOldTaxDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
            $total_tax = + ($detail -> weight * $detail -> gram_tax );
        }
    ...
    public function ExitOldTaxAccounting($id){ 
        $bill = ExitOldTax::find($id);
        $details = ExitOldTaxDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
    ...
    public function EnterOldAccounting($id)
    {
        ...
        if ($bill) {
            ...
            if($bill -> bill_type == 0)
            {
                $is_type = 'شراء ذهب كسر';
                $gold_purchase_account = $settings->purchase_old_account;
                $gold_stock_account = $settings->stock_old_account;
            }else{
                $is_type = 'شراء ذهب صافي';
                $gold_purchase_account = $settings->purchase_pure_account;
                $gold_stock_account = $settings->stock_pure_account;
            }
            //journal header
            $headerData = [
            ...
            if ($bill->total_money > 0) {
                $detailsData[] = [ 
                    'account_id' => $settings->purchase_account,
                    //'account_id' => $gold_purchase_account,
                    'debit' => $bill->total_money, 
                    'credit' => 0, 
                    'ledger_id' => 0,
                    'notes' => ''
    ...
    public function SimplifiedDebitAccounting($id){
        $bill = SimplifiedDebit::find($id);
        $details = SimplifiedDebitDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
            $total_tax = + ($detail -> weight * $detail -> gram_tax );
        }
        if($bill){
            ...
            $headerData = [
                'branch_id' => $bill->branch_id,
                'date' => $bill->date,
                'basedon_no' => $bill->serial_number,
                'basedon_id' => $id,
                'baseon_text' => 'اشعار مدين لفاتورة مبسطة ',
                'total_credit' => 0,
                'total_debit' => 0,
                'notes' => ''
            ];
    ...
    public function StandardDebitAccounting($id){
        $bill = StandardDebit::find($id);
        $details = StandardDebitDetails::where('bill_id' , '=' , $id) -> get();
        $total_tax = 0 ;
        foreach ($details as $detail){
            $total_tax = + ($detail -> weight * $detail -> gram_tax );
        }
        if($bill){
            ...
            $headerData = [
                ...
                'basedon_no' => $bill->serial_number,
                'basedon_id' => $id,
                'baseon_text' => 'اشعار مدين لفاتورة ضريبية ',
                'total_credit' => 0,
                'total_debit' => 0,
                'notes' => ''
            ];
    ...
    public function CatchGoldReciptAccounting($id)
    {
     
        $bill = CatchGoldRecipts::find($id);
        if ($bill) {
            $settings = AccountSetting::where('branch_id',$bill->branch_id)->first();
            if (!$settings)
                return;
            ...
            if( $bill->gold21 > 0){ 
                ...
                $detailsData[] = [ 
                    'account_id' => $settings->stock_old_account,
                    'debit' => $bill->gold21,
                    'credit' => 0,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => 'جرام ذهب عيار 21'
                ...
                $detailsData[] = [ 
                    'account_id' => $settings->stock_under_account,
                    'debit' => 0,
                    'credit' => $bill->gold21,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => 'جرام ذهب عيار 21'
    ...
    public function ReturnEnterWorkAccounting($id)
    {
        // purchase gold from factory (money - gold)
        $bill = EnterWork::find($id);
        ...
        if ($bill) {
           ...
           if($bill -> bill_type == 0){   
            ...
            $headerData = [
                ...
                'basedon_no' => $bill->bill_number,
                'basedon_id' => $id,
                'baseon_text' => 'مردود مشتريات ذهب مشغول',
                'total_credit' => 0,
                'total_debit' => 0,
                'notes' => ''
            ];
            ...
            if ($bill->net_money <> 0 and $bill->total_money <> 0) {
                $detailsData[] = [
                    'account_id' => $customerAccount, 
                    'debit' => ($bill->net_money * -1) - ($bill->made_total * -1),
                    'credit' => 0,
                    'ledger_id' => $bill->supplier_id,
                    'notes' => ''
                ];
    ...
    public function ReturnEnterOldAccounting($id)
    {
        ...
        if ($bill) {
            ...
            if($bill -> bill_type == 0)
            {
                $is_type = 'مردود مشتريات ذهب كسر';
                $gold_purchase_account = $settings->purchase_old_account;
                $gold_stock_account = $settings->stock_old_account;
            }else{
                $is_type = 'مردود مشتريات ذهب صافي';
                $gold_purchase_account = $settings->purchase_pure_account;
                $gold_stock_account = $settings->stock_pure_account;
            }
    ...
    public function getRols(){
        if(empty(Auth::user()->role_name == 'مدير النظام')) {
            return Role::orderBy('id', 'ASC')
                ->where('name','<>','مدير النظام')
                ->where('guard_name','Admin-web')
                ->get();
        }else{
            return Role::orderBy('id', 'ASC')->where('guard_name','Admin-web')->get();
        }
    }

    public function getRolsPluck(){
        if(empty(Auth::user()->role_name == 'مدير النظام')) {
            return Role::where('name','<>','مدير النظام')
                ->where('guard_name', 'admin-web')
                ->get()->pluck('name', 'name');
        }else{
            return  Role::where('guard_name', 'admin-web')
                ->get()->pluck('name', 'name');;
        }
    }
    ...
    public function getUsers(){
      ...
      if(Auth::user()->role_name == 'مدير النظام') {
            return User::all(); 
        }else if(!empty(Auth::user()->branch_id)) {
