                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                            <form method="POST" action="{{ route('store_pos') }}"
                                 id="pos_sales_form" class="needs-validation" novalidate>
                                @csrf
                                @method('POST')
                                <input type="hidden" name="user_id" value="{{Auth::user()->id}}"/>
                                <input type="hidden" name="uuid" id="uuid" value="{{$uuid}}"/>
                                <div class="row">
                                    <div class="card shadow mb-4 col-9 m-0 p-0">
                                        ...
                                        <div class="card-body pe-3 ps-5">
                                            <div class="row"> 
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label >{{ __('main.document_type') }}  
                                                        </label>
                                                        <select class="form-control"
                                                                name="document_type" id="document_type" required>
                                                            <option value="1" >{{__('main.new_gold')}}</option>
                                                            <option value="0" >{{__('main.old_gold')}}</option> 
                                                        </select>
                                                    </div>
                                                </div> 
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label class="pe-4">{{ __('main.clients') }} 
                                                        </label>
                                        ...
                                        <div class="card-body m-0 p-0">
                                            <div class="document_type1 m-0 p-0">
                                                <div class="row"> 
                                                    <div class="col-md-12 p-4" id="sticker">
                                    ...
                                    <div class="card shadow mb-4 col-3 m-0 p-0">
                                        <div class="card-header py-3">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row" style="align-items: center; margin-bottom: 10px;" hidden>
                                                <div class="col-6">
                                                    <label style="text-align: right;float: right;"> {{__('main.made_Value_t')}} </label>
                                                </div>
                                                <div class="col-6">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {  
        ...
        $(document).on('click', '#pos_sales_btn', function () {
            ...
            var net_after_discount = $('#net_after_discount').val(); 
    ...
    function AddRowToTable(karat , id ) {
        ...
        if(!repeate) {
            ...
            if(id == 'tbody0'){
                ...
                cell9.innerHTML = `<td><button type="button" class="btn btn-outline-danger deleteBtn0" value=" '+item.id+' ">
                ...
                cell7.innerHTML = `<td><button type="button" class="btn btn-outline-danger  deleteBtn2 " value=" '+item.id+' ">
                                        <i class="fa fa-trash"></i></button> </td>`;
    ...
    function getBillNo() {
        ...
        $.ajax({
            type: 'get',
            url: 'get_sales_pos_no/1/' + type+ '/' + branch_id,
            dataType: 'json',
        ...
        $.each(response, function (i, item) {
            if (item.item_type == 1 || item.item_type == 3) {
                if (item.state == 1) {
                    ...
                    $data += '<li class="select_product" data-item-id="' + item.id + '">' + item.code  + ' ( ' + item.name_ar+ ' ) </li>';
                }
    ...
    function loadItems() {
        ...
        $.each(sItems, function (i, item) {
            ...
            var tr_html= '<td class="text-center"><input type="hidden" name="item_id[]" value="' + item.id + '"> <strong>' + item.name_ar + '</strong> [' + (item.code) + ']<br> '+item.karat.name_ar+' </td>';
            tr_html += '<td><input type="text" class="form-control Count" name="count[]" value="1" required> </td>';
            tr_html += '<td><input type="number" step="any" min="0.01" class="form-control iNewWeight" name="weight[]" value="' + item.weight + '" required></td>';
        ...
        document.getElementById('items_count').value =items_count_val ;
